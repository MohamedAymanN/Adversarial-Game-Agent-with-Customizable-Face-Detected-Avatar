...
/*
    Screen At which player 2 Chose his Avatar
*/
MultiP_CharSelectP2_Screen::MultiP_CharSelectP2_Screen(string img_loc)
{
    img_loc1=img_loc;
}

MultiP_CharSelectP2_Screen::~MultiP_CharSelectP2_Screen(){}

void MultiP_CharSelectP2_Screen::LoadContent(){
...
void MultiP_CharSelectP2_Screen::UnloadContent(){}

void MultiP_CharSelectP2_Screen::Update(sf::RenderWindow &window,sf::Event event){
    ...
    if(inp.mouse_left_clicked()){
        ...
        if((back_btn_sprite).getGlobalBounds().contains(mouse)){
        ...
        }else if((prev_btn_sprite).getGlobalBounds().contains(mouse)){
            sf::sleep(sf::milliseconds(100));
            Screen_Manager::GetInst().addscreen(new Choose_Mode_Screen);
        }else if((bala7a_player_sprite).getGlobalBounds().contains(mouse)){
            system("D:");
            ...
            Screen_Manager::GetInst().addscreen(new Multi_Player_Screen(img_loc1,"Images/pf2"));
        }else if((kim_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Multi_Player_Screen(img_loc1,"Images/kim_player"));
        }else if((putin_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Multi_Player_Screen(img_loc1,"Images/putin_player"));
        }else if((sadam_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Multi_Player_Screen(img_loc1,"Images/sadam_player"));
        }else if((sharron_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Multi_Player_Screen(img_loc1,"Images/sharon_player"));
        }else if((trump_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Multi_Player_Screen(img_loc1,"Images/trump_player"));
        }
    }

}

void MultiP_CharSelectP2_Screen::Draw(sf::RenderWindow &window){
    window.draw(bg_sprite);
    window.draw(back_btn_sprite);
    window.draw(prev_btn_sprite);
