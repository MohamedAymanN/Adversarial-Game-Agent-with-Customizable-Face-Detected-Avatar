...
/*
 the Screen At which the user chose his Avatar in: quick play -> Single player mode
*/
SingleP_CharSelect_Screen::SingleP_CharSelect_Screen(){}

SingleP_CharSelect_Screen::~SingleP_CharSelect_Screen(){}

void SingleP_CharSelect_Screen::LoadContent(){
...
void SingleP_CharSelect_Screen::UnloadContent(){}

void SingleP_CharSelect_Screen::Update(sf::RenderWindow &window,sf::Event event){
    ...
    if(inp.mouse_left_clicked()){
        ...
        if((back_btn_sprite).getGlobalBounds().contains(mouse)){
            ...
            Screen_Manager::GetInst().addscreen(new Single_Player_Screen("Images/pf1"));
        }else if((kim_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Single_Player_Screen("Images/kim_player"));
        }else if((putin_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Single_Player_Screen("Images/putin_player"));
        }else if((sadam_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Single_Player_Screen("Images/sadam_player"));
        }else if((sharron_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Single_Player_Screen("Images/sharon_player"));
        }else if((trump_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Single_Player_Screen("Images/trump_player"));
        }
    }

}

void SingleP_CharSelect_Screen::Draw(sf::RenderWindow &window){
