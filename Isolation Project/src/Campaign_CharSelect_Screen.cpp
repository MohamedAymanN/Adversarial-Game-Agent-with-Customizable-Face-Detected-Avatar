...
/*
    The screen at which the player chose the its Avatar in the Campaign Mode
*/
int Campaign_CharSelect_Screen::AI_level=0;

Campaign_CharSelect_Screen::Campaign_CharSelect_Screen(){}

Campaign_CharSelect_Screen::~Campaign_CharSelect_Screen(){}

void Campaign_CharSelect_Screen::LoadContent(){
...
void Campaign_CharSelect_Screen::UnloadContent(){}

void Campaign_CharSelect_Screen::Update(sf::RenderWindow &window,sf::Event event){
    ...
    if(inp.mouse_left_clicked()){
        ...
        if((back_btn_sprite).getGlobalBounds().contains(mouse)){
            ...
            Screen_Manager::GetInst().addscreen(new Campaign_Play_Screen("Images/pf1"));
        }else if((kim_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Campaign_Play_Screen("Images/kim_player"));
        }else if((putin_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Campaign_Play_Screen("Images/putin_player"));
        }else if((sadam_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Campaign_Play_Screen("Images/sadam_player"));
        }else if((sharron_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Campaign_Play_Screen("Images/sharon_player"));
        }else if((trump_player_sprite).getGlobalBounds().contains(mouse)){
            Screen_Manager::GetInst().addscreen(new Campaign_Play_Screen("Images/trump_player"));
        }
    }

}

void Campaign_CharSelect_Screen::Draw(sf::RenderWindow &window){
